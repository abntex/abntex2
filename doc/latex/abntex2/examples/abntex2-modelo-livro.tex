...
\ifxetex
  ...
  \let\textspanish\undefined  % babel já definiu o comando \textspanish, por isso copiamos para um novo nome...
  \setotherlanguages{english,french,german,italian,spanish}  % ... para que \setotherlanguages de polyglossia possa (re)defini-lo
  \usepackage{fontspec}
...
\else
  ...
  % comandos de fontes
  \usepackage{cochineal}   % fonte principal (serifada)
  \usepackage[tabular]{sourcesanspro}  % fonte sem serifas
  \usepackage[lf,scale=0.85]{sourcecodepro}  % fonte monoespaçada
  \usepackage[cochineal,bigdelims,cmintegrals,vvarbb]{newtxmath}  % fonte matemática
  \usepackage[bb=ams,cal=cm]{mathalfa}  % personalização dos alfabetos \mathbb, \mathcal, \mathfrak e \mathscr
\fi
...
\else
~\vfill Este texto foi composto em Cochineal, de Michael Sharpe, que é uma
bifurcação de Crimson, de Sebastian Kosch; e Source Sans Pro, de Paul D.
Hunt sob a orientação de Robert Slimbach.
\fi
